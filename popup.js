function start() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }).then(function (tabs) {
        ...
        if (doc.querySelector("meta[property='og:title']").getAttribute("content") == "ChatGPT") {
            ...
            document.body = factCheck(doc.querySelector('div[data-test-render-count="1"]').getElementsByClassName("font-claude-message")[0].textContent)
...
function factCheck(input) {
    ...
    data = {
        "fact_checks": [
          {
            "llm_claim": "COVID-19 can lead to dehydration, so maintaining fluid intake is crucial",
            "check": "The articles don't specifically address hydration, but discuss general supportive therapy",
            "backup": {
              "quote": "The current management of COVID-19 is based generally on supportive therapy and treatment to prevent respiratory failure",
              "link": "https://pubmed.ncbi.nlm.nih.gov/32749914/"
            }
          },
          {
            "llm_claim": "If symptoms worsen (e.g., difficulty breathing, chest pain, confusion), seek medical attention immediately",
            "check": "This claim is supported by the literature, which identifies these as serious symptoms",
            "backup": {
              "quote": "Manifestations of COVID-19 include asymptomatic carriers and fulminant disease characterized by sepsis and acute respiratory failure. Approximately 5% of patients with COVID-19, and 20% of those hospitalized, experience severe symptoms necessitating intensive care",
              "link": "https://pubmed.ncbi.nlm.nih.gov/32648899/"
            }
          },
          {
            "llm_claim": "COVID-19 isolation can lead to anxiety or loneliness",
            "check": "Multiple studies confirm the mental health impact of COVID-19 isolation",
            "backup": {
              "quote": "The COVID-19 pandemic's physical restrictions and social distancing measures have affected each and every domain of life... Anxiety, depression, disturbances in sleep and appetite, as well as impairment in social interactions are the most common presentations",
              "link": "https://pubmed.ncbi.nlm.nih.gov/33810225/"
            }
          }
        ],
        "similarity_assessment": {
          "reasoning": "The AI response generally aligns with scientific literature regarding COVID-19 management and symptoms. It accurately reflects the importance of monitoring symptoms, seeking medical attention when needed, and acknowledging both physical and mental health impacts. However, some specific recommendations (like hydration and specific medications) aren't directly addressed in the provided articles. The response takes a more practical, patient-focused approach while maintaining accuracy with available scientific evidence.",
          "similarity_score": 80
        }
      }
...
function formatFactChecks(factChecks) {
    ...
    factcontainer.innerHTML = `
        <h1>Fact Checks</h1>
        <ul>
            ${factChecks.map(factCheck => formatFactCheck(factCheck)).join("")}
        </ul>
    `
